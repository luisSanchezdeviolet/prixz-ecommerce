...
class NextendSocialLoginPRO {
    ...
    public static $nslMinVersion = '3.1.15';
    ...
    public static function integrationBuddyPressLoginRestriction($externalInsertUserStatus, $socialUser, $user_data) {
        if (class_exists('BuddyPress', false) && function_exists('bp_core_signup_user')) {
            ...
            if (is_email($user_data['user_email'])) {
                ...
                add_action('bp_core_signups_after_add_backcompat', function ($user_id) {
                    /**
                     *
                     * When a new user is inserted by BuddyPress with the bp_core_signup_user() function, it updates the user_status to 2 after the user is inserted,
                     * However they don't clear the user cache so until the page is refreshed, the subsequent user queries will continue working with the default 0 value because of cache,
                     * which means that the login will be enabled even if the our Support login restrictions is turned on and the account is not activated.
                     *
                     * @see NSLDEV-627
                     */

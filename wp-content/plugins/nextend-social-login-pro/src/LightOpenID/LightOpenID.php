...
class LightOpenID {
    ...
    public $returnUrl, $required = array(), $optional = array(), $verify_peer = null, $capath = null, $cainfo = null, $cnmatch = null, $data, $oauth = array(), $curl_time_out = 30          // in seconds
    , $curl_connect_time_out = 30; // in seconds
    private $identity, $claimed_id;
    protected $server, $version, $trustRoot, $aliases, $identifier_select = false, $ax = false, $sreg = false, $setup_url = null, $headers = array(), $proxy = null, $user_agent = 'LightOpenID', $xrds_override_pattern = null, $xrds_override_replacement = null;
    ...
    protected function request($url, $method = 'GET', $params = array(), $update_claimed_id = false) {
        ...
        return $use_curl ? $this->request_curl($url, $method, $params, $update_claimed_id) : $this->request_streams($url, $method, $params, $update_claimed_id);
    }
    ...
    protected function get_provider_name($provider_url) {
        ...
        if (!empty($provider_url)) {
            $tokens = array_reverse(explode('.', parse_url($provider_url, PHP_URL_HOST)));
    ...
    protected function authUrl_v2($immediate) {
        ...
        if (!empty($this->oauth) && is_array($this->oauth)) {
            ...
            $params['openid.oauth.consumer'] = str_replace(array(
                'http://',
                'https://'
            ), '', $this->trustRoot);
    ...
    function getAttributes() {
        if (isset($this->data['openid_ns']) && $this->data['openid_ns'] == 'http://specs.openid.net/auth/2.0') { # OpenID 2.0
    ...
    /**
     * The following functions are NSL Specific functions!
     */
